# ============================================================
# COIN RADAR + AI BIRLESIK ONAYLI SURUM
# Kaynaklar: btcturk_sinyal (36).py + main (8).py
# Telegram sinyal politikasi: YALNIZCA Radar + Giris + AI = 🟢 AL
# ============================================================
...
# V5.4.6: Telegram canlı kategorileri. Yıldız şimdilik kapalıdır.
...
# === AI ASSISTANT TEKNIK MOTORU (main (8).py) ===
...
# === /AI ASSISTANT TEKNIK MOTORU ===


# === RADAR + AI BIRLESIK ONAY KATMANI ===
# Radar adayi once kendi kategori + giris kalitesi kapilarindan gecer.
# Sonra AI teknik motoru ikinci kilit olarak calisir.
# Telegram'a yalnizca karar == "🟢 AL" olan adaylar cikabilir.
def birlesik_ai_onayla(aday):
    teknik = teknik_analiz_hesapla(aday.get("symbol", ""))
    aday["teknik"] = teknik

    # AI Assistant'in bekledigi alan adlarini Radar verisinden besle.
    ai_aday = dict(aday)
    ai_aday["radar_skoru"] = aday.get("guc_skoru", aday.get("skor", 0))
    ai_aday["radar_kategori"] = aday.get("durum", "")
    ai_aday["erken_aday"] = False
    ai_aday["teknik"] = teknik

    karar = h_karar_hesapla(ai_aday)
    aday.update(karar)
    aday["radar_skoru"] = ai_aday["radar_skoru"]
    aday["radar_kategori"] = ai_aday["radar_kategori"]
    aday["ai_onayli"] = karar.get("karar") == "🟢 AL"
    return aday["ai_onayli"]


def birlesik_piyasa_sinyalini_onayla(symbol):
    """Ayni taramadaki son piyasa kaydini birleşik sistem sinyali olarak isaretle."""
    try:
        for kayit in reversed(piyasa_kayitlari):
            if kayit.get("symbol") == symbol:
                kayit["sinyal_var"] = True
                kayit["ai_onayli"] = True
                return
    except Exception as e:
        print("Birleşik piyasa sinyal işaretleme hatası:", e)
# === /RADAR + AI BIRLESIK ONAY KATMANI ===
...
while True:
    try:
        print()
        print("COIN RADAR AI V5.4.6 | 2 KATEGORİ | ERKEN ROKET → 3dk AI → SADECE AL")
        print("--------------------------------")
            ...
            try:
                ...
                try:
                    if piyasa_kayitlari and piyasa_kayitlari[-1].get("symbol") == symbol:
                        piyasa_kayitlari[-1]["radar_sinyal_var"] = (durum in TELEGRAM_KATEGORILERI and giris_uygun)
                        piyasa_kayitlari[-1]["sinyal_var"] = False
                        piyasa_kayitlari[-1]["ai_onayli"] = False
        ...
        if len(adaylar) == 0:
            ...
            for a in adaylar:
                ...
                # V5.4.6: Yıldız/Yıldız Adayı canlı kategori değildir.
                # Profil yalnızca DNA/gelecekteki gerçek Yıldız modeli için kaydedilir.
                ...
                if havuz_kaydi is not None:
                    ...
                    if not a.get("giris_uygun", False):
                        ...
                        if durum == "🚀 Roket Adayı":
                            # Erken adayın amacı zaten oluşumu erkenden görmek.
                            # Burada öldürmek yerine risk bilgisini 3 dk AI takibine taşı.
                            print(
                                f"🌱 Erken Roket giriş riski: {symbol} | "
                                f"Giriş {a.get('giris_kalitesi', 0)}/100 | {neden} → AI takip edecek"
                            )
                        else:
                            print(
            ...
            if len(gosterilecekler) == 0:
                ...
                for sira, a in enumerate(gosterilecekler, start=1):
                    ...
                    satir += (
                        ...
                        f"🤖 AI Onayı: 🟢 AL | {a.get('ai_skoru', 0)}/100\n"
                    ...
                    teknik = a.get("teknik") or {}
                    if teknik:
                        ema_yon = "Yukarı ✅" if teknik.get("ema20") is not None and teknik.get("ema50") is not None and teknik.get("ema20") > teknik.get("ema50") else "Aşağı"
                        macd_yon = "Pozitif ✅" if teknik.get("macd_hist") is not None and teknik.get("macd_hist") > 0 else "Negatif"
                        satir += (
                            f"Teknik: EMA {ema_yon} | RSI {teknik.get('rsi')} | ADX {teknik.get('adx')} | MACD {macd_yon}\n"
                        )
